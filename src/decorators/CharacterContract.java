...
public class CharacterContract extends CharacterDecorator implements CharacterService {
	...
	public void GoLeft() {
		...
		CellContent contentenbas = super.getEnvi().getCellContent(wdt, hgt - 1);
		...
		CellContent contentagauche = super.getEnvi().getCellContent(wdt - 1, hgt);
		super.GoLeft();
		checkInvariant();
		if (natureEnBas != Cell.EMP && natureEnBas != Cell.HOL) {
			if (!(super.getHgt() == hgt)) {
		...
		if ((nature_at_pre != Cell.LAD && nature_at_pre != Cell.HDR)
				&& (natureEnBas == Cell.EMP || natureEnBas == Cell.HOL) && contentenbas.getCharacter().size() == 0) {
		...
		if (wdt > 0
				...
				&& (base.contains(super.getEnvi().getCellNature(wdt, hgt - 1))
						...
						|| Cell.HDR == super.getEnvi().getCellNature(wdt, hgt)
						|| a> 0)
				&& contentagauche.getCharacter().size() == 0) {
	...
	public void GoUp() {
		...
		int wdt = super.getWdt();
		CellContent contentenbas = super.getEnvi().getCellContent(wdt, hgt - 1);
		CellContent contentEnHaut = super.getEnvi().getCellContent(wdt, hgt + 1);
		...
		if (!((natureEnHaut == Cell.HDR || natureEnHaut == Cell.MTL || natureEnHaut == Cell.PLT)
				|| contentEnHaut.getCharacter().size() > 0)) {
	...
	public void GoRight() {
		...
		Cell natureEnBas = super.getEnvi().getCellNature(wdt, hgt - 1);
		Cell nature_at_pre = super.getEnvi().getCellNature(wdt, hgt);
		CellContent contentadroite = super.getEnvi().getCellContent(wdt + 1, hgt);
		super.GoRight();
		checkInvariant();
		if (natureEnBas != Cell.EMP && natureEnBas != Cell.HOL) {
			if (!(super.getHgt() == hgt)) {
		...
		if (wdt == super.getEnvi().getWidth() - 1) {
			if (!(super.getWdt() == wdt)) {
				throw new PostCondError("perso left malgre x=max");
			}
		}
		...
		if ((wdt < super.getEnvi().getWidth() - 1)
				&& !(Cell.MTL.equals(super.getEnvi().getCellNature(wdt + 1, hgt))
						|| Cell.PLT.equals(super.getEnvi().getCellNature(wdt, hgt)))
				&& contentadroite.getCharacter().size() == 0) {
			if (!(super.getWdt() == wdt + 1)) {
	...
	public void GoDown() {
		...
		if (hgt != 0 && (nature_at_pre == Cell.LAD || nature_at_pre == Cell.HDR || nature_at_pre == Cell.EMP)
				&& (natureEnBas == Cell.HOL || natureEnBas == Cell.LAD || natureEnBas == Cell.HDR
						|| natureEnBas == Cell.EMP)) {
			if (!(super.getHgt() == hgt - 1 && super.getWdt() == wdt)) {
